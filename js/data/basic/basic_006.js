...
(window.PROBLEMS_REGISTRY = window.PROBLEMS_REGISTRY || []).push({
  ...
  // ── 正解順序の制約（partialOrder）────────────────────────────────
  // id:1（sort の第1行）→ id:2（lambda 続き）は必ず連続
  // id:2（sort完了）→ id:3（count/cur初期化）: sort完了後に初期化
  // id:3 → id:4（for）→ id:5（if）: ループ構造は順序固定
  // id:5 の中で id:6（count++）と id:7（cur=e）は互いに独立 → どちらが先でも正解
  // id:8（内側の }）は id:6・id:7 両方の後
  // id:8 → id:9（外側の }）→ id:10（return）→ id:11（関数 }）
  partialOrder: [
    [0, 1], [1, 2],             // 関数定義 → sort 1行目 → sort 2行目(lambda)
    [2, 3],                     // sort後 → count/cur初期化
    [3, 4], [4, 5],             // for → if
    [5, 6], [5, 7],             // if以降: count++ と cur=e はどちらが先でも可
    [6, 8], [7, 8],             // } は count++ と cur=e の両方の後
    [8, 9], [9, 10], [10, 11], // 残りは順序固定
  ],
  hints: [
