...
(window.PROBLEMS_REGISTRY = window.PROBLEMS_REGISTRY || []).push({
  ...
  difficulty: 2,
  language: 'cpp',
  description: '【動的計画法（DP）とは】「大きな問題を小さな部分問題に分けて解き、その結果を表（テーブル）に記録しながら再利用する」技術です。一度計算した答えを覚えておくことで、同じ計算を何度もしなくて済むため、劇的に高速になります。\n\nフィボナッチ数列（1,1,2,3,5,8,13...）は「前の2つの数を足したもの」ですが、ナイーブな再帰では同じ計算が爆発的に増えます。DP では計算結果をテーブルに保存することで O(N) で解けます。\n\n動的計画法（DP）を使ってフィボナッチ数列の第 N 項を O(N) で求めよ。再帰では指数時間かかるが、DP なら線形時間で解ける。',
  inputFormat: {
    params: [
